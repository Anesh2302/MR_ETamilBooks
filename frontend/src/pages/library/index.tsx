...
import { FiBook, FiSearch, FiDownload, FiEye, FiChevronRight, FiBookmark, FiClock, FiX } from 'react-icons/fi';
...
export default function Library() {
  ...
  useEffect(() => {
    ...
    getBooks(params).then((res) => setBooks(res.data)).catch(() => {}).finally(() => setLoading(false));
  }, [selectedCat, search, language]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-in-up">
        ...
        <div className="flex items-center space-x-2">
          <select className="w-32 rounded-xl px-3 py-2.5 text-sm" style={{ background: 'var(--input-bg)', border: '1px solid var(--border-color)', color: 'var(--text-primary)' }} value={language} onChange={(e) => setLanguage(e.target.value)}>
        ...
        {showSearches && recentSearches.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-1 rounded-xl shadow-xl z-10 p-2" style={{ background: 'var(--card-bg)', border: '1px solid var(--border-color)' }}>
            ...
            {recentSearches.map((q, i) => (
              <button key={i} onClick={() => { setSearch(q); setShowSearches(false); handleSearchSubmit(); }} className="flex items-center gap-2 w-full px-2 py-1.5 text-sm rounded-lg hover:bg-white/5" style={{ color: 'var(--text-primary)' }}>
      ...
      <div className="flex space-x-2 overflow-x-auto scrollbar-hide pb-2 animate-fade-in">
        <button
          ...
          className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-medium transition-all ${!selectedCat ? 'bg-tamil-600 text-white shadow-md' : 'border hover:bg-white/10'}`}
        ...
        {categories.map((cat: any) => (
          <button
            ...
            className={`px-4 py-2 rounded-full whitespace-nowrap text-sm font-medium transition-all ${selectedCat === cat.id ? 'bg-tamil-600 text-white shadow-md' : 'border hover:bg-white/10'}`}
      ...
      {loading ? (
        <div className="text-center py-12">
          <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-tamil-500 mx-auto"></div>
          <p className="mt-4 animate-pulse" style={{ color: 'var(--text-secondary)' }}>Loading books...</p>
        </div>
      ) : books.length === 0 ? (
              ...
              <div className="w-full h-48 bg-gradient-to-br from-tamil-900/30 to-orange-900/20 rounded-lg mb-4 flex items-center justify-center overflow-hidden relative">
                ...
                {prog && (
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/10">
                    <div className="h-full bg-tamil-500 rounded-r" style={{ width: `${Math.min(100, Math.round(prog.progress * 100))}%` }} />
                  </div>
                )}
              </div>
